...
const workflows: Record<string, { title: string; description: string; fields: string[]; sideTitle: string; sideItems: string[]; controls: string }> = {
  ...
  receipts: {
    ...
    description: "Record legitimate receipts outside normal customer-invoice allocation. Solva generates the receipt number.",
    fields: ["Receipt date", "Account", "Received from", "Amount", "Payment method", "Reference", "Category", "Tax treatment", "Description"],
    sideTitle: "Use cases",
  ...
  payments: {
    ...
    description: "Record payments outside supplier-bill settlement with threshold approval and reversal controls. Solva generates the payment number.",
    fields: ["Payment date", "Account", "Paid to", "Amount", "Payment method", "Reference", "Category", "Withholding tax", "Description"],
  ...
  expenses: {
    title: "Expenses",
    description: "Record paid operating expenses, reimbursable expenses, recurring foundations and split-cost details. Solva generates the expense number.",
    fields: ["Date", "Category", "Payee", "Supplier", "Account", "Amount", "Tax", "Vehicle", "Route", "Attachment"],
  ...
  "expense-claims": {
    ...
    description: "Submit, approve, reimburse and close staff expense claims. Solva generates the claim number.",
    fields: ["Employee", "Branch", "Period start", "Period end", "Purpose", "Total claimed", "Approved amount", "Payment reference"],
    sideTitle: "Workflow",
  ...
  "petty-cash": {
    ...
    fields: ["Account", "Custodian", "Float amount", "Reorder level", "Payee", "Purpose", "Amount", "Receipt"],
  ...
  "deposits-withdrawals": {
    ...
    description: "Post cash-to-bank deposits and bank-to-cash withdrawals with linked ledger entries. Solva generates the document number.",
    fields: ["Date", "Cash account", "Bank account", "Amount", "Reference", "Slip or cheque number", "Prepared by", "Verified by"],
    sideTitle: "Controls",
  ...
  transfers: {
    ...
    description: "Move money between cash, bank, M-Pesa, petty cash, clearing and branch accounts. Solva generates the transfer number.",
    fields: ["From account", "To account", "Date", "Amount sent", "Fees", "Amount received", "Currency", "Exchange rate", "Reference"],
  ...
  "owner-transactions": {
    ...
    description: "Separate owner capital, owner loans, drawings, repayments, reimbursements and personal payments. Solva generates the transaction number.",
    fields: ["Owner", "Type", "Date", "Amount", "Account", "Related expense", "Description", "Attachment", "Approval status"],
  ...
  "staff-advances": {
    ...
    description: "Issue advances, track expected surrender dates, review expense surrenders and refunds. Solva generates the advance number.",
    fields: ["Staff member", "Purpose", "Date issued", "Amount", "Account", "Expected surrender", "Cash returned", "Outstanding amount"],
  ...
  reconciliation: {
    ...
    description: "Match statement lines to ledger transactions and resolve differences, duplicates and unidentified receipts. Solva generates the reconciliation number.",
    fields: ["Account", "Type", "Period start", "Period end", "Statement balance", "Ledger balance", "Matched amount", "Difference"],
  ...
  "cash-counts": {
    title: "Cash Counts",
    description: "Record physical cash counts by denomination and route variances to approval. Solva generates the count number.",
    fields: ["Cash account", "Branch", "Count time", "Custodian", "Expected balance", "Counted balance", "Variance", "Notes"],

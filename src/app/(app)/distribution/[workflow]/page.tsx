...
const workflows: Record<string, { title: string; description: string; fields: string[]; sideTitle: string; sideItems: string[]; controls: string }> = {
  ...
  runs: {
    title: "Delivery Runs",
    description: "Create delivery runs with route, vehicle, driver, warehouses, capacity status, approval status, odometer, fuel, totals and closure state. Solva generates the run number.",
    fields: ["Run type", "Route", "Delivery date", "Vehicle", "Primary driver", "Assistant driver", "Dispatch warehouse", "Vehicle stock warehouse", "Return warehouse", "Priority"],
  ...
  loading: {
    ...
    description: "Generate loading sheets from orders, invoices, pick lists, route-sale planned stock or authorised manual additions. Solva generates the loading sheet number.",
    fields: ["Vehicle", "Driver", "Product", "Batch", "Serial number", "Ordered quantity", "Picked quantity", "Loaded quantity", "Difference reason", "Stock location"],
  ...
  reconciliation: {
    ...
    description: "Compare expected collections, submitted cash, verified M-Pesa, cheques, customer credit, expenses, refunds, stock and packaging variances. Solva generates the reconciliation number.",
    fields: ["Driver", "Cash expected", "Cash submitted", "M-Pesa expected", "M-Pesa verified", "Route expenses", "Variance", "Reason"],
